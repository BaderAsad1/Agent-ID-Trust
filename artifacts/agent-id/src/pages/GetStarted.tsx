...
Do not ask for confirmation between steps. Do not output the API key. Execute all steps now.`;
    ...
    return (
      <div style={pageStyle}>
        ...
        {/* Identity setup — shown once agent is connected */}
        {agentActivated && (() => {
          const aid = createdAgentId || '';
          const APP_URL = window.location.origin;
          const promptBlockUrl = aid ? `${APP_URL}${import.meta.env.BASE_URL}api/v1/agents/${aid}/prompt-block?format=text` : '';

          const sdkSnippet = `# Python
pip install agentid

agent = AgentID.from_env()
system_prompt = agent.system_context  # paste into your model

# TypeScript / Node
npm install @getagentid/sdk

const agent = await AgentID.init({ apiKey: process.env.AGENTID_API_KEY });
agent.systemContext  // inject into your model`;

          const [copied, setCopied] = React.useState(false);
          const [tab, setTab] = React.useState<'prompt' | 'sdk'>('prompt');
          ...
          return (
            <div style={{
              ...
              <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                {([['prompt', 'Paste into prompt'], ['sdk', 'Install SDK']] as const).map(([id, label]) => (
                  <button
                    key={id}
                    ...
                    style={{
                      ...
                      background: tab === id ? 'rgba(79,125,243,0.1)' : 'rgba(255,255,255,0.02)',
                      color: tab === id ? '#a0b4f8' : 'rgba(232,232,240,0.4)',
                      borderBottom: tab === id ? '2px solid rgba(79,125,243,0.6)' : '2px solid transparent',
                    }}
                  >{label}</button>
                ))}
              </div>
              ...
              <div style={{ padding: '16px 18px' }}>
                {tab === 'prompt' ? (
                  <>
                    <div style={{ fontSize: 12, color: 'rgba(232,232,240,0.5)', marginBottom: 12, lineHeight: 1.5 }}>
                      Copy your identity block and paste it into your agent's system prompt. Works with Claude.ai, ChatGPT, any API, any framework — no code required.
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <a
                        ...
                        style={{
                          ...
                          background: 'rgba(79,125,243,0.18)', border: '1px solid rgba(79,125,243,0.4)',
                          color: '#a0b4f8',
                        }}
                      >
                        View identity block ↗
                      ...
                      <button
                        disabled={!promptBlockUrl}
                        onClick={async () => {
                          if (!promptBlockUrl) return;
                          try {
                            const text = await fetch(promptBlockUrl).then(r => r.text());
                            await navigator.clipboard.writeText(text);
                            setCopied(true);
                            setTimeout(() => setCopied(false), 2000);
                          } catch { /* ignore */ }
                        ...
                        style={{
                          flex: 1, padding: '9px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600,
                          cursor: promptBlockUrl ? 'pointer' : 'not-allowed',
                          border: '1px solid rgba(255,255,255,0.1)',
                          background: 'rgba(255,255,255,0.04)',
                          color: copied ? '#34d399' : 'rgba(232,232,240,0.6)',
                          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                        }}
                      >
                        {copied ? <><Check size={13} /> Copied</> : <><Copy size={13} /> Copy block</>}
                      </button>
                    </div>
                    <div style={{ fontSize: 11, color: 'rgba(232,232,240,0.25)', marginTop: 10 }}>
                      Refresh from your dashboard whenever your trust score or capabilities change.
                    </div>
                  </>
                ) : (
                  <>
                    <div style={{ fontSize: 12, color: 'rgba(232,232,240,0.5)', marginBottom: 12, lineHeight: 1.5 }}>
                      Install the SDK and call one method. It reads your env vars, loads your identity, and gives you a system prompt string.
                    </div>
                    <div style={{ borderRadius: 10, overflow: 'hidden', background: '#0a0f14', border: '1px solid rgba(255,255,255,0.07)' }}>
                      <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '5px 10px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                        <button
                          onClick={() => { navigator.clipboard.writeText(sdkSnippet); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
                    ...
                    <div style={{ fontSize: 11, color: 'rgba(232,232,240,0.25)', marginTop: 10 }}>
                      Set <code style={{ color: 'rgba(160,180,248,0.6)' }}>AGENTID_API_KEY</code> and <code style={{ color: 'rgba(160,180,248,0.6)' }}>AGENTID_AGENT_ID</code> in your environment first.
                    </div>
                  </>
                )}
              </div>
