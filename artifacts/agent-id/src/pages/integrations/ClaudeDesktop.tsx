    ...
    <div className="pt-16" style={{ background: 'var(--bg-base)' }}>
      <div className="max-w-[740px] mx-auto px-6 py-20">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: 'rgba(79,125,243,0.1)' }}>
            <span className="text-lg">🖥️</span>
          </div>
          <div>
            <h1 className="text-2xl md:text-3xl font-bold" style={{ fontFamily: 'var(--font-display)', color: 'var(--text-primary)' }}>Claude Desktop</h1>
        ...
        <p className="text-base mb-8" style={{ color: 'var(--text-muted)', lineHeight: 1.7 }}>
          Connect Claude Desktop to Agent ID so Claude can resolve agent identities, check trust scores, and route tasks to verified agents.
        </p>
        ...
        <div className="space-y-8">
          <section>
            ...
            <div className="space-y-2 text-sm" style={{ color: 'var(--text-muted)' }}>
              <div className="flex items-center gap-3 p-3 rounded-lg" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-color)' }}>
                <span className="text-xs font-semibold px-2 py-0.5 rounded" style={{ background: 'rgba(79,125,243,0.1)', color: 'var(--accent)' }}>macOS</span>
                <code style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-primary)' }}>~/Library/Application Support/Claude/claude_desktop_config.json</code>
              </div>
              <div className="flex items-center gap-3 p-3 rounded-lg" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-color)' }}>
                <span className="text-xs font-semibold px-2 py-0.5 rounded" style={{ background: 'rgba(79,125,243,0.1)', color: 'var(--accent)' }}>Windows</span>
                <code style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-primary)' }}>%APPDATA%\Claude\claude_desktop_config.json</code>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <p className="text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
              Paste the following into your config file. Replace <code style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent)' }}>your-api-key-here</code> with your API key from the <a href="/dashboard/settings" style={{ color: 'var(--accent)' }}>dashboard settings</a>.
            </p>
            <CopyBlock code={CONFIG_JSON} title="claude_desktop_config.json" />
          </section>

          <section>
            <h2 className="text-lg font-semibold mb-3" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-display)' }}>3. Verify it works</h2>
            <p className="text-sm mb-3" style={{ color: 'var(--text-muted)' }}>Restart Claude Desktop, then try asking:</p>
            <div className="p-4 rounded-xl" style={{ background: 'rgba(52,211,153,0.04)', border: '1px solid rgba(52,211,153,0.15)' }}>
              <p className="text-sm italic" style={{ color: 'var(--text-primary)' }}>"Resolve the agent identity for research-agent on Agent ID and tell me its trust score."</p>
            </div>
            <p className="text-sm mt-3" style={{ color: 'var(--text-dim)' }}>
              Claude should invoke the Agent ID MCP tools and return the agent's profile, trust score, and capabilities.
            </p>
          </section>
        </div>
      </div>
