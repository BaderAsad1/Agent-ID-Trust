...
import { AlertCircle, RefreshCw, User, Bot, CheckCircle } from 'lucide-react';
...
export function HumanProfile() {
  ...
  const [profile, setProfile] = useState<HumanData | null>(null);
  ...
  const fetchProfile = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${BASE}/humans/${handle}`);
      if (!res.ok) throw new Error('Profile not found');
      const data = await res.json();
      setProfile(data);
    } catch {
      setError(`No profile with handle "${handle}" exists.`);
    } finally {
      setLoading(false);
    }
  }, [handle]);
  ...
  if (error) return (
    <div className="pt-16" style={{ background: '#050711', minHeight: '100vh' }}>
      <div className="max-w-[600px] mx-auto px-6 py-20 text-center">
        ...
        <h3 style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: 24, fontWeight: 700, color: '#e8e8f0', marginBottom: 8 }}>Profile not found</h3>
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 20 }}>
            {profile.avatarUrl ? (
              <img src={profile.avatarUrl} alt={profile.displayName} style={{ width: 96, height: 96, borderRadius: '50%', objectFit: 'cover', border: '3px solid rgba(52,211,153,0.3)' }} />
            ) : (
              <div style={{
                width: 96, height: 96, borderRadius: '50%',
                background: 'linear-gradient(135deg, rgba(52,211,153,0.3), rgba(79,125,243,0.3))',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                border: '3px solid rgba(52,211,153,0.3)',
              }}>
                <User size={40} style={{ color: 'rgba(232,232,240,0.6)' }} />
              </div>
            )}
          </div>
          ...
          <div style={{
            ...
            color: 'rgba(79,125,243,0.6)', marginBottom: 20,
            display: 'inline-flex', alignItems: 'center', gap: 6,
            background: 'rgba(79,125,243,0.06)', padding: '5px 12px', borderRadius: 6,
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 32, marginTop: 8 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: 28, fontWeight: 700, color: '#e8e8f0' }}>{profile.agentCount}</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: '0.12em', color: 'rgba(232,232,240,0.3)', textTransform: 'uppercase' }}>Public Agents</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px 80px' }}>
        ...
        {profile.agents.length === 0 ? (
          <div style={{ padding: '60px 20px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 14, color: 'rgba(232,232,240,0.3)' }}>No public agents yet.</p>
          </div>
        ) : (
          <div style={{
            display: 'grid',
