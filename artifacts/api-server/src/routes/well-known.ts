...
router.get("/.well-known/agent-registration", async (_req: Request, res: Response) => {
  ...
  res.json({
    ...
    baseDomain: BASE_DOMAIN,
    endpoints: {
      register: { url: `${APP_URL}/api/v1/programmatic/agents/register`, method: "POST" },
      verify: { url: `${APP_URL}/api/v1/programmatic/agents/verify`, method: "POST" },
      resolve: { url: `${APP_URL}/api/v1/resolve/{handle}`, method: "GET" },
      discovery: { url: `${APP_URL}/api/v1/resolve`, method: "GET" },
      reverseResolve: { url: `${APP_URL}/api/v1/resolve/reverse`, method: "POST" },
      handleCheck: { url: `${APP_URL}/api/v1/handles/check`, method: "GET" },
      handlePricing: { url: `${APP_URL}/api/v1/handles/pricing`, method: "GET" },
      agentProfile: { url: `${APP_URL}/api/v1/p/{handle}`, method: "GET", description: "Public agent profile (human-readable, no auth required)" },
      agentIdentity: { url: `${APP_URL}/api/v1/public/agents/{agentIdOrHandle}`, method: "GET", description: "Machine-readable agent identity document (no auth required)" },
      marketplaceListings: { url: `${APP_URL}/api/v1/marketplace/listings`, method: "GET" },
      jobs: { url: `${APP_URL}/api/v1/jobs`, method: "GET" },
      healthCheck: { url: `${APP_URL}/api/healthz`, method: "GET" },
      llmsTxt: { url: `${APP_URL}/api/llms.txt`, method: "GET" },
    },
    authentication: {
    ...
    pricing: {
      tiers: [
        ...
        { characters: "5+", pricePerYear: "Included", description: "Standard handle — 1 included with Starter or Pro plan (choose and register one from your dashboard; no additional payment required). Enterprise: custom/sales-led." },
      ],
      marketplaceFee: "2.5%",
      gracePeriodDays: 90,
      plans: [
