...
http.setResponseCallback(http.expectedStatuses({ min: 200, max: 299 }, 404, 429));

const BASE_URL = __ENV.BASE_URL || "https://getagent.id/api/v1";
const TEST_HANDLE = __ENV.TEST_HANDLE || "testagent";

const machineHeaders = {
  "User-Agent": "k6-stress-test/1.0 (+https://k6.io)",
...
export default function () {
  ...
  check(handleAvailable, {
    "handle check 200 or 429": (r) => r.status === 200 || r.status === 429,
  });
